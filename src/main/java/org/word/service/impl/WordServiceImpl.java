...
public class WordServiceImpl implements WordService {
        ...
        try {
            ...
            if (paths != null) {
                ...
                while (it.hasNext()) {
                    ...
                    table.setHeaderList(processHeaderList(parameters));
                    table.setPathList(processPathList(parameters));
                    table.setQueryList(processQueryList(parameters));
                    
                    Table tmpTable=processRequestList(parameters,definitinMap);
                    table.setRequestList(tmpTable.getRequestList());
                    table.setRequestModelAttr(tmpTable.getRequestModelAttr());
     ...
     * @return
     */
    private List<Request> processHeaderList(List<LinkedHashMap> parameters) {
  ...
  private List<Request> processQueryList(List<LinkedHashMap> parameters) {
  List<Request> requestList = new ArrayList<>();
  if (!CollectionUtils.isEmpty(parameters)) {
      for (Map<String, Object> param : parameters) {
          Object in = param.get("in");
  ...
  private List<Request> processPathList(List<LinkedHashMap> parameters) {
	  List<Request> requestList = new ArrayList<>();
	  if (!CollectionUtils.isEmpty(parameters)) {
	      for (Map<String, Object> param : parameters) {
	          Object in = param.get("in");
...
//    }
    ...
    private Table processRequestList(List<LinkedHashMap> parameters, Map<String, ModelAttr> definitinMap) {
    	Table table=new Table();
        List<Request> requestList = new ArrayList<>();
        ModelAttr requestModel=null;
        if (!CollectionUtils.isEmpty(parameters)) {
            for (Map<String, Object> param : parameters) {
                Object in = param.get("in");
                Request request = new Request();
                ...
                if (in != null && "body".equals(in)) {
                    ...
                    String refValue=ref == null ? "{}" : ref.toString();
                    request.setParamType(refValue);
                    
                    if(ref!=null) {
                    	requestModel=this.processResponseModelAttrs(param, definitinMap);
                    }
        ...
        requestModel=requestModel==null?new ModelAttr():requestModel;
        if(requestModel.getProperties()==null) {
        	requestModel.setProperties(new ArrayList<ModelAttr>());
        }
        table.setRequestModelAttr(requestModel);
     ...
     * @return
     */
    private ModelAttr processResponseModelAttrs(Map<String, Object> obj, Map<String, ModelAttr> definitinMap) {
        Map<String, Object> schema = (Map<String, Object>) obj.get("schema");
